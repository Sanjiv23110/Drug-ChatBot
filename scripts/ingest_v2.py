...
def main():
    ...
    # 4. Get Checkpoint (already ingested files)
    logger.info("Checking for already ingested files...")
    already_ingested = get_already_ingested_files(qm)
    logger.info(f"Found {len(already_ingested)} ingested items.")
    ...
    for idx, xml_file in enumerate(xml_files, 1):
        ...
        try:
            ...
            if is_file_ingested(metadata, already_ingested):
                ...
                print(f"DEBUG: SKIPPED {basename}") # Explicit print
                stats['skipped'] += 1
                continue
            ...
            print(f"DEBUG: CAUGHT EXCEPTION for {basename}: {e}") # Explicit print
            ...
            try:
                ...
                print(f"DEBUG: Failed to write to log: {fe}")
